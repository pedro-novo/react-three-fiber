...
export default function Experience() {
  ...
  const instances: InstancedRigidBodyProps[] = useMemo(() => {
    ...
    for (let i = 0; i < cubesCount; i++) {
      const instance: InstancedRigidBodyProps = {
        key: "instance_" + i,
    ...
    <>
      ...
      <Physics debug={false} gravity={[0, -9.81, 0]}>
        ...
        <RigidBody type="fixed">
          <CuboidCollider mass={2} args={[5, 2, 0.1]} position={[0, 1, 5.1]} />
          <CuboidCollider mass={2} args={[5, 2, 0.1]} position={[0, 1, -5.1]} />
          <CuboidCollider
            mass={2}
            args={[5, 2, 0.1]}
...
// --- STRESS TEST ---
// Adding as many cubes as we can until the performance drops.
// Matrix4 is a combination of position, rotation and scale.
// They're used to move the vertices according to the object transformation.
// When we change the position, rotation or scale of an object, Three.js
// will calculate the Matrix4 automatically before rendering it.
// With the apporach we're taking (useEffect), we're rendering all
// the cubes in one draw call.

// INSTANCE RIGID BODIES
// We can get rid of the cubes reference and useEffect since
// InstanceRigidBodies will take care of creating and sending the
// matrices to instanceMesh.
